...
router.get('/beers/:id', (req, res) => {
  res.send(JSON.stringify(BEERS[req.params.id]));
});
...
router.put('/dogs/:id', (req, res) => {
  var dogIndex = BEERS.findIndex((dog) => dog.id.toString() == req.params.id.toString());
  dogId = BEERS[dogIndex].id;
  BEERS[dogIndex] = req.body.dog;
  BEERS[dogIndex].id = dogId;
  res.send(JSON.stringify(BEERS[dogIndex]));
});

router.delete('/dogs/:id', (req, res) => {
  var dogIndex = BEERS.findIndex((dog) => dog.id.toString() == req.params.id.toString());
  BEERS.splice(dogIndex, 1);
  res.status(200).send({});
});

function generateId() {
  return BEERS[BEERS.length - 1].id + 1;
}
